...
```cpp
const int MAX_HARI = 1000;            // Maksimal hari yang dapat diinput
...
|----|--------|--------|-----------|
| 1 | `inputSuhu()` | ✅ | Input data suhu harian dengan validasi |
| 2 | `bubbleSort()` | ✅ | Mengurutkan suhu ascending (rendah ke tinggi) |
| 3 | `hitungRataRata()` | ✅ | Menghitung rata-rata suhu dengan akumulasi |
| 4 | `cekTren()` | ✅ | Analisis tren suhu menggunakan counting approach |
| 5 | `tampilkanHasil()` | ✅ | Menampilkan hasil analisis dalam format terstruktur |
| 6 | `main()` | ✅ | Fungsi utama program dengan validasi input |
...
- **Kapasitas Maksimal:** 1000 hari
- **Library:** `iostream`, `iomanip`, `string`
- **Memory Usage:** ~8-20 KB (tergantung jumlah hari)

## 📝 Catatan Penting

1. **Input Suhu:** Gunakan bilangan bulat (integer) untuk suhu dalam Celcius
2. **Validasi Input:** Program menangani input invalid dengan baik menggunakan cin.fail()
3. **Analisis Tren:** Menggunakan data ASLI sebelum sorting untuk akurasi
4. **Format Output:** Menggunakan `iomanip` untuk tampilan profesional
5. **Urutan Pemanggilan:** inputSuhu() → Copy Array → cekTren() → bubbleSort() → hitungRataRata()
